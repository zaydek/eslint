...
// One-off values not in the shared token set, kept local to this stress test.
const PANEL_RAISED = '#232330';
const LINE_STRONG = '#383848';
const SHADOW_MENU = '0 18px 48px rgba(0, 0, 0, 0.55)';
const SHADOW_DIALOG = '0 18px 48px rgba(0, 0, 0, 0.6)';
...
const styles = stylex.create({
  // Runtime color — StyleX's answer to the raw stylesheet's inline `--dot`.
  // A dynamic style function takes the value at call time; no inline `style`.
  ...
  MenuItem: {
    ...
    borderRadius: 7,
    fontSize: 13,
    // Base UI sets [data-highlighted]; match it with :is() so specificity stays flat.
    color: {
    ...
    backgroundColor: {
      default: 'transparent',
      ':is([data-highlighted])': PANEL_RAISED,
    },
    cursor: 'default',
    userSelect: 'none',
    outline: 'none',
  ...
  // The centerpiece: every animatable property is keyed on [data-phase].
  // transition-* are multi-value lists that themselves change per phase.
  Confirm: {
    position: 'fixed',
    zIndex: 81,
    left: '50%',
  ...
  ConfirmActionsBtnIsSecondary: {
    color: tokens.textDim,
    backgroundColor: { default: PANEL_RAISED, ':hover': tokens.line },
...
export function ColumnActions({
  ...
  return (
    <>
      {/* dynamic color flows through the dynamic style function */}
      <button {...stylex.props(styles.Dot, styles.DotColor(dotColor))} />
      ...
      <div {...stylex.props(styles.Menu)}>
        ...
        <div {...stylex.props(styles.MenuItem, styles.MenuItemIsDanger)} data-danger>
          Delete
        </div>
      </div>

      <div {...stylex.props(styles.Backdrop)} />
      <div {...stylex.props(styles.Confirm)} data-phase={phase}>
        ...
        <div {...stylex.props(styles.ConfirmActions)}>
          <button {...stylex.props(styles.ConfirmActionsBtn, styles.ConfirmActionsBtnIsSecondary)}>
            Cancel
          </button>
          <button
